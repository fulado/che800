...
# 车辆读取线程，浙江
def get_vehicle_thread_zj(v_queue):
...
# 违章查询线程
def query_thread(v_queue, city):
    print('%s auto query start.' % city)
    while True:
        try:
...
def query_vio_auto():
    ...
    # 创建2个上海车辆查询线程
    for i in range(2):
        t_query_thread_sh = Thread(target=query_thread, args=(vehicle_queue_sh, 'shanghai_%d' % (i + 1)))
        t_query_thread_sh.start()
